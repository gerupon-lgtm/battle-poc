(function () {
  ...
  // ============================================================
  // 連続案(combo)用: カウント→攻撃→カウント→防御 を「単一テンポの拍グリッド」で
  // 途切れず演奏する。音楽は攻撃/防御の本編のみ、カウント中はカウント音のみ。
  ...
  function comboCountHits(startBeat, bars) {
    ...
    for (let b = 0; b < bars; b++) {
      const barStart = startBeat + b * 4;
      if (b === bars - 1) { for (let q = 0; q < 4; q++) hits.push({ beat: barStart + q, label: q + 1, strong: q === 0 }); }
      else { hits.push({ beat: barStart, label: 1, strong: true }); hits.push({ beat: barStart + 2, label: 2, strong: false }); }
    }
    return hits;
  }
  // opts: { songId, bpm, countAttackBars, attackBars, countDefenseBars, defenseBars, onPhase(ph), onEnd() }
  function startCombo(opts) {
    opts = opts || {};
    comboClear();
    const A = ensureAudio();
    ...
    const defCountStart = atkEnd, defStart = atkEnd + cDef, defEnd = defStart + def;
    const total = defEnd;
    const startTime = A.currentTime + SETTINGS.startLeadSec;
    const countEl = $("count-in");

    // 音(audio時計): カウント音＋本編の音楽
    ...
    for (const h of atkHits.concat(defHits)) {
      (function (hh) { at(hh.beat, function () { if (countEl) { countEl.hidden = false; countEl.textContent = String(hh.label); } }); })(h);
    }
